...
if(isset($_POST['add_to_cart'])){
    ...
    if(mysqli_num_rows($check_cart_numbers) > 0){
        ...
        mysqli_query($conn, "INSERT INTO `cart`(user_id, pid, name, price, quantity, image) VALUES('$user_id', '$product_id', '$product_name', '$product_price', '$product_quantity', '$product_image')") or die('Interogare esuata!');
...
if(isset($_GET['delete'])){
    ...
    mysqli_query($conn, "DELETE FROM `wishlist` WHERE id = '$delete_id'") or die('Interogare esuata!');
    header('location:wishlist.php');
}

if(isset($_GET['delete_all'])){
    mysqli_query($conn, "DELETE FROM `wishlist` WHERE user_id = '$user_id'") or die('Interogare esuata!');
    header('location:wishlist.php');
}
...
<section class="wishlist">
    ...
    <?php
        $grand_total = 0;
        $select_wishlist = mysqli_query($conn, "SELECT * FROM `wishlist` WHERE user_id = '$user_id'") or die('Interogare esuata!');
        if(mysqli_num_rows($select_wishlist) > 0){
            while($fetch_wishlist = mysqli_fetch_assoc($select_wishlist)){
    ?>
    <form action="" method="POST" class="box">
        <a href="wishlist.php?delete=<?php echo $fetch_wishlist['id']; ?>" class="fas fa-times" onclick="return confirm('Doresti sa stergi acest produs din Wishlist?');"></a>
        ...
        <div class="price">RON <?php echo $fetch_wishlist['price']; ?>/-</div>
    ...
    $grand_total += $fetch_wishlist['price'];
        ...
        echo '<p class="empty">Wishlist-ul tau este gol!</p>';
    }
    ?>
    </div>

    <div class="wishlist-total">
        <p>Total : <span>RON <?php echo $grand_total; ?>/-</span></p>
        <a href="shop.php" class="option-btn">Continua cumparaturile</a>
        <a href="wishlist.php?delete_all" class="delete-btn <?php echo ($grand_total > 1)?'':'disabled' ?>" onclick="return confirm('Doresti sa stergi tot din Wishlist?');">Sterge tot</a>
    </div>
